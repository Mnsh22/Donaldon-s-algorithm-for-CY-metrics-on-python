...
def weight_list():
    ...
    for i in range(len(sample)):
        w = 1 / (25 * (abs(cont[i]) ** 8) * (detlist[i]) * ((1j/2)) )
...
def T_map_iteration(h_inv, ff, wml, secmatrixgen, n_iter):
    for _ in range(n_iter):
        T_h = ff * (1/len(sample)) * sum( ( (secmatrixgen[i] * wml[i]) / (np.einsum("mn,mn", h_inv, secmatrixgen[i])) ) for i in range(len(sample)))
        print(T_h)
        h_inv = np.linalg.inv(T_h).T #indices
...
def K_ijbar_builder():
    ...
    for i in range(N_t):
        k_ijbar = np.einsum('ia,ab,jb->ij', ds[i], h_inv, np.conj(ds[i]))
        K_ij_list.append(k_ijbar)
    return K_ij_list

K_ijbar_list = K_ijbar_builder()
...
def K_0_builder():
    ...
    for i in range(N_t):
        k_0 = 1 / ( np.einsum("mn,mn", h_inv, secmatrixgen[i]) )
        k_0_list.append(k_0)
    return k_0_list
...
def Kbar_ijbar_builder():
    ...
    ds = ds_list
    for i in range(N_t):
        Kbar_ijbar = np.einsum('nm,jn,im->ji', h_inv, ds[i], np.matrix.conj(ds[i]))
...
def volume_error_kahler_metric_list():
    ...
    for i in range(N_t): # from notes ML 4CY
        g = (1/(K * np.pi)) * ( (K_0_list[i] * K_ijbar_list[i]) - ( ((K_0_list[i]) * (K_0_list[i])) * (np.outer(K_i_list[i], np.matrix.conj(K_ibar_list[i]))) ) )
...
def error_Vol_K():
    ...
    evk = (1/N_t) * sum( ( (-(1j/2)**3) * 6 * (det_metric_EVK_list[i] / OmOmbar_list[i] ) * w_M_list[i]) for i in range(N_t))
    return evk
...
def sigma_builder():
    ...
    sigma = (1/(N_t*EVCY)) * sum( ((abs( 1 - ( (-(1j/2)**3) * 6 * (det_metric_EVK_list[i]/EVK) / ((OmOmbar_list[i])/EVCY) ) ) ) * w_M_list[i]) for i in range(N_t))
    return sigma
...
K_ijkbar_list = K_ijkbar_builder()
...
Kbar_jkibar_list = Kbar_jkibar_builder()
...
def double_derivative_metric_builder():
    ...
    K_ijk = K_ijkbar_list
    K_ij = K_ij_list
    Kbar_jki = Kbar_jkibar_list

    for i in range(N_t):
        didj_gkl = ((1/(np.pi*K)) *
                    ( ((K_0[i]) * (K_ijkl[i]))
                    - ((K_0[i])**2) * ( (np.einsum('ij,kl->ijkl', K_ijbar[i], K_ijbar[i]))
                    + (np.einsum('ik,jl->ijkl', K_ij[i], np.matrix.conj(K_ij[i]).T)) #CHECK IF BAR = HERMITIAN
                    ...
                    + (np.einsum('i,k,jl->ijkl', K_i[i], K_i[i], np.matrix.conj(K_ij[i]).T))
...
def Ricci_flatness_error():
    ...
    R = R_list
    eR = ((EVK ** (1/3))/(N_t * EVCY)) * (-(1j/2)**3) * 6 * sum( ( (det_metric_EVK_list[i] / (OmOmbar_list[i])) * (abs(R[i])) * (w_M_list[i]) ) for i in range(N_t) )
    return eR
