...
def extra_coordinates_fixing(coord_fix_fn, extras):
    for i in range(len(extras)):
        ...
        # NB: Need to equal this to a number or Int problem
...
def extra_coordinates_fixing(coord_fix_fn, extras):
    for i, b in enumerate(extras):
        rhs = -sum(coord_fix_fn[i][k]**5 for k in range(5) if k != b)
        # candidate 5th roots
        bases = [rhs**(1/5) * np.exp(2j*np.pi*m/5) for m in range(5)]
        # choose the one closest to the current value (from the line intersection)
        orig = coord_fix_fn[i][b]
        coord_fix_fn[i][b] = bases[np.argmin([abs(r - orig) for r in bases])]
    return coord_fix_fn
'''
...
def weight_list(cont, detlist, sample):
    ...
    for i in range(len(sample)):
        w = 1 / ( 25 * ((abs(cont[i])) ** 8) * (6 * detlist[i]) )
...
def Error_sigma(EVK, EVCY, det, OmOmbar):
    sigma = (1/(N_t * EVCY)) * sum( (abs(1 - ( (6 * det[i]/EVK) / (OmOmbar[i]/EVCY) )) * w_M_list[i]) for i in range(N_t) )
    return sigma
...
def Matematica_sigma(EVK, EVCY, det, OmOmbar):
    sig = (1/N_t) * (sum( (abs(1 - ((6 * det[i]/OmOmbar[i]) * (w_M_list[i]/EVK) )) ) for i in range(N_t)) )
    return sig

Mat_sigma = Matematica_sigma(EVK, EVCY, det_CY_metric, OmOmbar_list)
